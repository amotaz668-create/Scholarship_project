...
import {
  ...
  ResolvedApplicationRequirement
...
@Component({
  ...
  template: `
    <section class="page-hero compact page-container application-flow-hero">
      ...
      <h1>
        Complete your <span>application.</span>
      </h1>

      <p>
        Profile information is reused automatically.
        Add only missing or scholarship-specific details.
      </p>
    </section>
    ...
    @else if (preparation(); as data) {
      ...
      <form
        class="application-flow-layout page-container"
        [formGroup]="form"
        (ngSubmit)="save(false)"
      >
        ...
        <main class="application-flow-main">
          ...
          <section class="content-card">
            ...
            <div class="section-heading">
              ...
              <h2>Already in your passport</h2>
            </div>

            <div class="prepared-data-grid">

              @for (
                requirement of profileRequirements();
                ...
                @if (requirement.origin === 'profile') {
                  ...
                  <div>
                    ...
                    <b>
                      <bdi>
                        {{ displayValue(requirement.value) }}
                      </bdi>
                    </b>
              ...
              @empty {
                <p class="muted">
                  This scholarship does not request additional reusable
                  profile fields.
                </p>
              }
          ...
          @if (editableRequirements().length) {
            ...
            <section class="content-card">
              ...
              <div class="section-heading">
                <p class="eyebrow">ADDITIONAL INFORMATION</p>
                <h2>Complete the missing details</h2>
              </div>

              <div class="dynamic-requirements">

                @for (
                  requirement of editableRequirements();
                  track requirement.key
                ) {

                  <label
                    [class.full-width]="requirement.type === 'textarea'"
                  >
                    ...
                    @switch (requirement.type) {
                      ...
                      @case ('textarea') {
                        <textarea
                          rows="6"
                          [formControlName]="requirement.key"
                        ></textarea>
                      }
                      ...
                      @case ('select') {
                        <select [formControlName]="requirement.key">
                      ...
                      @case ('boolean') {
                        <span class="confirmation-control">
              ...
              @if (hasEditableProfileFields()) {
                ...
                <label class="check-label save-profile-choice">

                  <input
                    type="checkbox"
                    [formControl]="saveToProfile"
                  >
                ...
                <article [class.ready]="documentFor(type)">
                  ...
                  <div>
                    ...
                    <div>
                      ...
                      <small>
                        {{ documentFor(type)?.fileName || 'No file selected' }}
                      </small>
...
export class ApplicationCompleteComponent {
  ...
  readonly preparation =
    signal<ApplicationPreparation | null>(null);
  ...
  readonly selectedDocumentIds =
    signal<Set<string>>(new Set());

  readonly saveToProfile =
  ...
  readonly profileRequirements = computed(() =>
    this.preparation()?.requirements.filter(
      (item) =>
        item.source === 'profile' &&
        item.type !== 'document'
    ) ?? []
  ...
  readonly editableRequirements = computed(() =>
    this.preparation()?.requirements.filter(
      (item) =>
        item.type !== 'document' &&
        (
  ...
  readonly hasEditableProfileFields = computed(() =>
    this.editableRequirements().some(
  ...
  readonly documentTypes = computed(() => {
    ...
    return [
      ...new Set([
        ...(data.scholarship.requiredDocuments ?? [])
  ...
  readonly localComplete = computed(() => {

    this.formState();

    return (
      this.form.valid &&
      this.documentTypes().every(
        (type) =>
          Boolean(this.documentFor(type)) &&
          this.selectedDocumentIds().has(
            this.documentFor(type)!._id
          )
      )
    );
  ...
  readonly missingLabels = computed(() => {
    ...
    const labels =
      this.editableRequirements()
        .filter(
          (item) =>
            item.required &&
            this.form.get(item.key)?.invalid
        )
        .map(
          (item) =>
            this.requirementLabel(item)
        );

    for (
      const type of this.documentTypes()
    ) {

      if (
        !this.documentFor(type) ||
        !this.selectedDocumentIds().has(
          this.documentFor(type)!._id
        )
      ) {
  ...
  save(review: boolean): void {
    ...
    this.applications
      .update(this.id, {
        ...
        documents:
          (this.preparation()?.availableDocuments ?? [])
            .filter(
              (document) =>
                this.selectedDocumentIds().has(
                  document._id
    ...
    this.documents
      ...
      .subscribe({
        ...
        next: ({ data }) => {
          ...
          // Replace the currently selected document of the same type.
          // This prevents the old file and the newly uploaded file from
          // being attached to the same application at the same time.
          this.selectedDocumentIds.update((ids) => {
            const next = new Set(ids);

            for (const document of this.preparation()?.availableDocuments ?? []) {
              if (
                document.type === type &&
  ...
  requirementLabel(
    requirement: ResolvedApplicationRequirement
  ): string {
  ...
  private bind(
    ...
    for (
      ...
      if (
        !data.availableDocuments.some(
          (document) =>
            document.type === type &&
            selected.has(document._id)
        )
      ) {

        const reusable =
          data.availableDocuments.find(
            (document) =>
              document.type === type
          );
    ...
    for (
      ...
      if (
        requirement.type === 'document' ||
        (
          requirement.source === 'profile' &&
          requirement.origin === 'profile'
        )
      ) {
        continue;
      }
