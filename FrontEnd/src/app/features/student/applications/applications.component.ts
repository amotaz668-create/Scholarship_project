...
@Component({ selector: 'app-applications', standalone: true, imports: [RouterLink, ApplicationJourneyComponent, UiStateComponent], template: `
  ...
  <section class="page-container"><div class="tab-bar">@for (tab of tabs; track tab.value) { <button type="button" [class.active]="filter() === tab.value" (click)="filter.set(tab.value)">{{ tab.label }} <span>{{ tab.value ? count(tab.value) : applications().length }}</span></button> }</div>
    ...
    @else { <div class="application-list">@for (item of filtered(); track item._id) { <div><app-application-journey [application]="item" /><div class="application-actions"><span>Application ID: <bdi>{{ item._id }}</bdi></span>@if (item.status === 'draft') { <a class="button primary small" [routerLink]="item.isComplete ? ['/applications', item._id, 'review'] : ['/applications', item._id, 'complete']">{{ item.isComplete ? 'Review & Submit' : 'Continue application' }}</a> } @if (['submitted','under_review','missing_documents'].includes(item.status)) { <button class="button danger small" type="button" (click)="withdraw(item)">Withdraw</button> }</div></div> }</div> }
  </section>` })
export class ApplicationsComponent {
  ...
  readonly tabs: Array<{ label: string; value: ApplicationStatus | '' }> = [{ label: 'All', value: '' }, { label: 'Draft', value: 'draft' }, { label: 'Applied', value: 'submitted' }, { label: 'Under review', value: 'under_review' }, { label: 'Missing documents', value: 'missing_documents' }, { label: 'Accepted', value: 'accepted' }, { label: 'Rejected', value: 'rejected' }, { label: 'Withdrawn', value: 'withdrawn' }];
  ...
  withdraw(item: ScholarshipApplication): void { if (!confirm(this.i18n.translate('Withdraw this application? This cannot be reversed.'))) return; this.api.withdraw(item._id).subscribe({ next: ({ data }) => this.replace(data), error: (error: unknown) => this.error.set(apiErrorMessage(error)) }); }
