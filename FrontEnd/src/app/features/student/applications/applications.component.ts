...
@Component({
  ...
  imports: [
    RouterLink,
    ApplicationJourneyComponent,
    UiStateComponent
  ],
  template: `
    <section class="page-hero compact page-container">
      <p class="eyebrow">MY APPLICATIONS</p>
      <h1>Routes currently <span>in motion.</span></h1>
      <p>
    ...
    <section class="page-container">
      ...
      <div class="tab-bar">
        @for (tab of tabs; track tab.value) {
          ...
          >
            {{ tab.label }}
            <span>
              {{ tab.value ? count(tab.value) : applications().length }}
            </span>
          </button>
        }
      </div>
      ...
      @if (loading()) {
        ...
        <div class="skeleton journey-skeleton"></div>

      } @else if (!filtered().length) {

        <app-ui-state
          title="No applications in this stage"
          message="Your matching application journeys will appear here."
        />
              ...
              <div class="application-actions">
                ...
                @if (item.status === 'draft') {
                  ...
                  <a
                    ...
                    [routerLink]="
                      item.isComplete
                        ? ['/applications', item._id, 'review']
                        : ['/applications', item._id, 'complete']
                    "
                  >
                    {{ item.isComplete ? 'Review & Submit' : 'Continue application' }}
                  </a>

                }

                <!-- Missing Documents -->
                @if (item.status === 'missing_documents') {

                  <a
                    class="button primary small"
                    [routerLink]="['/applications', item._id, 'complete']"
                  >
                    Update Documents
                  </a>
                ...
                <!-- Withdraw -->
                @if (
                  ['submitted', 'under_review', 'missing_documents']
...
export class ApplicationsComponent {
  ...
  readonly filter = signal<ApplicationStatus | ''>('');
  ...
  withdraw(item: ScholarshipApplication): void {
    if (
      !confirm(
        this.i18n.translate(
    ...
    this.api.withdraw(item._id).subscribe({
  ...
  private replace(updated: ScholarshipApplication): void {
